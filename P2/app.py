...
# class AcademicPaperQA:
#     def __init__(self, model_path="./models/llama-2-7b-chat.Q4_0.gguf"):
#         """Initialize the Academic Paper Q&A system with llama.cpp optimized for CPU"""
#         self.data_dir = Path("./papers")
#         self.storage_dir = Path("./storage")
...
# #     def _setup_models(self):
# #         """Setup LLM and embedding models optimized for CPU with 16GB RAM"""
# #         try:
# #             # Initialize LLM via llama.cpp with CPU-optimized settings
# #             self.llm = LlamaCPP(
# #                 model_path=self.model_path,
# #                 temperature=0.3,
# #                 max_new_tokens=512,  # Reduced for faster generation
# #                 context_window=2048,  # Reduced for memory efficiency
# #                 generate_kwargs={
# #                     "top_p": 0.9,
# #                     "top_k": 40,
# #                     "repeat_penalty": 1.1,
...
# #             # Configure global settings with reduced resource usage
# #             Settings.llm = self.llm
# #             Settings.embed_model = self.embed_model
# #             Settings.chunk_size = 512  # Smaller chunks for memory efficiency
# #             Settings.chunk_overlap = 50  # Reduced overlap
            
# #             logger.info(f"Models initialized successfully with CPU-optimized settings")
...
#                 context_window=4096,  # Increased context
#                 generate_kwargs={
#                     "top_p": 0.9,
#                     "top_k": 40,
#                     "repeat_penalty": 1.1,
...
#                     "f16_kv": True,    # Changed to True for better performance
#                     "use_mlock": False, # Changed to False to avoid memory issues
#                     "use_mmap": True,
#                     "verbose": True,   # Enable verbose logging
#                 },
#                 verbose=True,
#                 # SIMPLIFIED SYSTEM PROMPT - This is key!
#                 system_prompt="You are a helpful research assistant. Answer questions about academic papers clearly and directly based on the provided context."
#             )
...
#             Settings.llm = self.llm
#             Settings.embed_model = self.embed_model
#             Settings.chunk_size = 1024  # Increased chunk size
...
#     def create_index(self, documents: List[Document], save_index: bool = True):
#         """Create vector index from documents with memory-efficient settings"""
#         try:
#             if not documents:
#                 raise ValueError("No documents provided for indexing")
...
#     def _create_query_engine(self):
#         """Create query engine with settings optimized for CPU"""
#         try:
#             if not self.index:
#                 raise ValueError("No index available for query engine")
...
#             retriever = VectorIndexRetriever(
#                 index=self.index,
#                 similarity_top_k=3  # Reduced for faster processing
#             )
...
#     #         # Create memory buffer with reduced token limit for efficiency
...
#     #             retriever=VectorIndexRetriever(
#     #                 index=self.index,
#     #                 similarity_top_k=3  # Reduced for efficiency
#     #             ),
#     #             memory=memory,
#     #             llm=self.llm,
#     #             context_prompt=(
...
#             # Create chat engine with SIMPLE context prompt
#             self.chat_engine = CondensePlusContextChatEngine.from_defaults(
#                 retriever=VectorIndexRetriever(
#                     index=self.index,
#                     similarity_top_k=3
...
#         """Ask a question using chat engine (always conversational mode)"""
#         if not self.is_ready:
#             return {"error": "System not ready. Please process papers first."}
...
#             # Always use chat engine for conversational context
#             response = self.chat_engine.chat(question)
#             answer = str(response)
...
#     # Model selection in sidebar (CPU-optimized models)
#     st.sidebar.header("⚙️ Configuration")
#     model_options = {
#         "Llama 2-7B Chat Q4_0 (Recommended)": "./models/llama-2-7b-chat.Q4_0.gguf",
#         "Mistral 7B Instruct Q4_0": "./models/mistral-7b-instruct-v0.1.Q4_0.gguf",
#         "CodeLlama 7B Q4_0": "./models/codellama-7b-instruct.Q4_0.gguf",
#         "Llama 2-7B Chat Q3_K_M (Faster)": "./models/llama-2-7b-chat.Q3_K_M.gguf",
#         "TinyLlama 1.1B Q4_0 (Lightest)": "./models/tinyllama-1.1b-chat-v1.0.Q4_0.gguf"
#     }
...
#     3. Recommended: llama-2-7b-chat.Q4_0.gguf (~3.8GB)
    
#     **CPU Optimization:**
#     - Using Q4_0 quantization for speed
#     - Reduced context window (2048)
#     - CPU-only inference
#     """)
    
#     # Initialize session state
#     if 'qa_system' not in st.session_state or st.session_state.get('current_model') != model_path:
#         with st.spinner(f"Initializing system with {selected_model}..."):
#             try:
#                 st.session_state.qa_system = AcademicPaperQA(model_path=model_path)
#                 st.session_state.current_model = model_path
#                 st.session_state.papers_loaded = False
#                 st.success(f"System initialized with {selected_model}!")
#             except Exception as e:
#                 st.error(f"Error initializing system: {e}")
#                 st.info("Make sure the GGUF model file exists at the specified path.")
#                 st.stop()
...
#     # Display current model info
#     st.sidebar.info(f"**Current model:** {selected_model}")
#     st.sidebar.success("💬 **Always in Conversational Mode**")
...
#             if question_to_ask:
#                 with st.spinner("Thinking..."):
#                     result = st.session_state.qa_system.ask_question(question_to_ask)
